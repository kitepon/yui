...
- **ダイキンへLANから直結** — 対応エアコンをcloudを通さず操作。実機が申告するmodeと温度範囲だけを表示します。風向は自動・固定・スイング。自動運転の相対温度と外気温も扱います。外気温はエアコンのセンサー項目としてオートメーションの条件に使えます。
- **場面とautomation** — 「おやすみ」で複数機器を動かし、時刻やsensor値をきっかけにserver側で実行します。画面に出ている設定は、触らなくても保存します。センサーのしきい値は小数点第一位まで入れられます。範囲内なら、いまの設定と違うときだけ送ります（設定を読み返せない赤外線には使えません）。同じ機器を含むオートメーションが同時に満たされたら、一覧の上だけがその機器を動かします。
- **iPhoneでも使う** — [SwiftUI製アプリ](ios/README.md)は開発用ビルドで使えます。Safariからホーム画面への追加もできます。Echo対応（Alexa Smart Home）は実装済みで、公式スキル「結ホーム」はAmazonの審査中です（2026-08-23提出）。公開までの間も、自分のserverで動かす場合は[docs/alexa.md](docs/alexa.md)の手順で自分のスキルとして使えます。
...
Smart Life の初回同期と新しい機器の鍵取得には Tuya IoT Platform が必要です。鍵を保存した後は、同じ LAN で見つかったローカル版 3.1 / 3.3 の機器を直接読み書きします。名乗りが一時的に途切れても、直近30分以内に読めた宛先で LAN を試します。LAN 読取に失敗してもクラウドには切り替えず、カードに「LAN不可」と理由を出します。LAN に見つからない機器と版 3.4 / 3.5 はクラウドを使います。
...
結と同じLANにいる対応機器をクラウドを通さず直接読み書きします。機器のIPを手で設定する必要はありません。受け役とファイアウォールの条件は[配備手順](deploy/README.md)を確認してください。
...
- **ダイキンの検証範囲** — 2020年うるさらX 1機種で実測しています。同世代でも他機種は未検証です。風向は自動・固定・スイングまで。固定羽根の多段位置、パワフル、ストリーマは出さない。
- **クラウド家電** — 表に無い機種、掃除機・加湿器、Nature Remo の左右風向は未対応です。Smart Life は機器が持たない操作を出せません。
- **Smart Life のクラウド経路** — LAN に見つからない機器とローカル版 3.4 / 3.5 は Tuya の IoT Core が必要です。試用枠を使い切ると、この経路と新しい機器の同期は失敗します。保存済みの鍵で動く LAN 機器には影響しません。
- **SwitchBot ボット** — クラウドから押すにはハブが要る。サーバーに Bluetooth がある結では `YUI_SWITCHBOT_BLE_URL` で直結できる。押すモードは `press`。
- **upgrade保証** — `v0.x`ではdatabaseや設定の長期互換範囲をまだ保証していません。Release noteで変更点を確認してください。
- iPhoneアプリのApp Store公開とCloudflareへの実移転は後続工程です。
