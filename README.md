...
- **iPhoneのホーム画面へ** — Safariからホーム画面へ追加すれば、appのように使えます。Echo対応（Alexa Smart Home）はコードとして備えていますが、公開スキルは未提供です。自分のserverで動かす場合は[docs/alexa.md](docs/alexa.md)の手順で自分のスキルとして使えます。
