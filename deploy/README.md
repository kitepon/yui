...
| `YUI_LAN_GW` | 任意 | 容器の橋の番地（既定 `172.16.240.1`）。受け役が TCP 6668 を中継する口 |
...
機器は LAN へ UDP 6666（3.1）/ 6667（3.3 以降）で名乗る。`tuya-lan` がホストの LAN にその口を直接開き、受け取った datagram を容器（`YUI_APP_ADDR`）へ渡し、機器への TCP 6668 もホスト側で中継する。容器から家の LAN へは届かない。ルーターの再起動でアドレスやリンクが変わると、口を閉じて開き直す。ウェブの待受は `127.0.0.1` のまま。`YUI_SUBNET` を変えたときは `YUI_LAN_GW` にその橋の番地（既定 `172.16.240.1`）を書く。
