...
- 公式hosted版のWeb契約は、1つの家あたり月額100円または年額1,000円（税込）、初回30日間無料で提供し、Stripeで購入・管理する。
  ...
  購入開始は結アカウントごとの単一予約と両決済元の最新状態確認で制御し、他方の購入手続き中も新規購入を許さない。
...
- Smart Life（Tuya）の IoT Platform 登録は、初回同期と新しい機器の鍵取得のために使う。同期は機器一覧の `local_key` / `localKey` と `specifications` の `dp_id` / `dpId` 対応を `credentials.tuyaLocal` に暗号化して保存する。機器の居場所は機器自身の名乗り（UDP 6666 / 6667）を結が聞いて覚え、人は宛先を書かない。
- 同じ LAN にいて版が 3.1 か 3.3 の Smart Life 機器は、センサー更新も操作も LAN（Tuya ローカルプロトコル）で行う。名乗りが途切れても直近30分以内に LAN で読めた宛先を使う。LAN 読取に失敗した機器はクラウドへ切り替えず、失敗理由を示す。LAN の宛先が無い機器と 3.4 / 3.5 の機器はクラウドを使う。
- Smart Life の経路は機器カードと接続タブに出す。直近の LAN 成功は「LAN」、失敗は「LAN不可」とし、古い成功記録だけで LAN の印を残さない。この経路は家ごとの鍵で動くので `YUI_LAN_OWNER` の制限は受けない。
