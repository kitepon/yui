...
- Smart Life（Tuya）の IoT Platform 登録は初回同期のためだけに使う。同期は機器一覧の `local_key` と `specifications` の dp 対応を `credentials.tuyaLocal` に暗号化して保存する。機器の居場所は機器自身の名乗り（UDP 6666 / 6667）を結が聞いて覚え、人は宛先を書かない。同じ LAN にいて版が 3.1 か 3.3 の機器は、センサー更新も操作も LAN（Tuya ローカルプロトコル）で行い、クラウドへは問い合わせない。LAN に居ない機器と 3.4 / 3.5 の機器はクラウドのまま。経路は機器カードと接続タブに出し、静かに古い値を残さない。この経路は家ごとの鍵で動くので `YUI_LAN_OWNER` の制限は受けない。
