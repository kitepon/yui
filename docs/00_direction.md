...
- SwitchBot ボットの押すモードは `press`。クラウドから押すにはハブが要る。サーバーの Bluetooth で押す口（`YUI_SWITCHBOT_BLE_URL`）があるときは、押すボットはクラウドへ送らず直結する。ハブ無しクラウドはオフラインとして出す。押すボットは状態を持たないので、オートメーションは条件に入ったときだけ押し、入っているあいだは再送しない。
