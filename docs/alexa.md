...
Lambda のタイムアウトは **8 秒**（Alexa の上限）。Smart Life のクラウド経路はトークン→状態→コマンドの 3 往復があり、既定の 3 秒では入りきらない。LAN 対応機器は機器への読取→操作で動く。
...
- 特定の機器だけ動かない → 接続先ごとに実装が違う。Nature Remo はセンサー・エアコン（温度・モード・風量・風向）・照明・学習済み赤外線、SwitchBot はボット・コンセント・照明・カーテン・鍵・センサー・IR エアコン、Smart Life は機器が持つ DP（スイッチ・明るさ・カーテン・エアコンなど）。ダイキン直結は温度・モード・風量・風向（自動・固定・スイング）・自動運転の相対温度。表に無い機種と、結に kind が無い掃除機・加湿器は詳細操作を出さない。未対応は黙って成功を返さずエラーを出す
- Smart Life だけ `IoT Core` のエラーになる → 結の接続タブで機器ごとの経路を見る。「LAN不可」なら表示された LAN の失敗理由を確認する。「クラウド」なら Tuya の試用枠と機器のローカル版を確認する。保存済みの鍵で LAN に接続できる 3.1 / 3.3 機器は IoT Core を使わない
