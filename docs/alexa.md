...
- 特定の機器だけ動かない → 接続先ごとに実装が違う。Nature Remo はセンサー・エアコン（温度・モード・風量・風向）・照明・学習済み赤外線、SwitchBot はボット・コンセント・照明・カーテン・鍵・センサー・IR エアコン、Smart Life は機器が持つ DP（スイッチ・明るさ・カーテン・エアコンなど）。表に無い機種と、結に kind が無い掃除機・加湿器は詳細操作を出さない。未対応は黙って成功を返さずエラーを出す
