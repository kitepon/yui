...
import { googleAuthConfigured } from "./google";
import { GROK_PROVIDERS } from "./providers";
import { getSqlite } from "../server/sqlite";
import { nodeSqliteDialect } from "../server/sqlite-dialect";
import {
  GROK_ISSUER_DEFAULT,
...
const productAuth = Boolean(env("BETTER_AUTH_URL") || env("YUI_SQLITE_PATH") || emailAndPasswordEnabled);
const grokClientId = env("GROK_AUTH_CLIENT_ID") ?? (productAuth ? undefined : PREVIEW_CLIENT_ID);
const grokClientSecret = env("GROK_AUTH_CLIENT_SECRET") ?? (productAuth ? undefined : PREVIEW_CLIENT_SECRET);
...
export const authConfigured =
  !authDisabled &&
  (emailAndPasswordEnabled || googleOn || Boolean(grokClientId && grokClientSecret));
...
const database = {
  dialect: nodeSqliteDialect(() => getSqlite()),
...
export const auth = betterAuth({
  ...
  // Encrypt OAuth tokens at rest. Google is a trusted first-party identity so
  // the same email as an existing password user links instead of
  // `account_not_linked`. Grok broker ids stay listed only while that plugin
